...
## Core Features
-   **Unified CLI (`skills`):** A custom Go-based CLI tool to manage the entire ecosystem. It replaces the previous `jig` tool and shell-based sync scripts. Features include:
...
## User Experience
-   **CLI-First:** All interactions are driven through the command line using \`mise\` tasks or the \`jig\` binary.
